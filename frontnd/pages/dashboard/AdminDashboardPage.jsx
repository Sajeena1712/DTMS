...
export default function AdminDashboardPage() {
  ...
  const aiStats = useMemo(() => {
    return tasks.reduce(
      (acc, task) => {
        const score = Number(task.aiEvaluation?.predictedScore);
        const thresholds = task.aiEvaluation?.thresholds || { autoApproveThreshold: 80, autoRejectThreshold: 50 };
        const reviewByAi = String(task.review?.reviewedBy || "").toUpperCase() === "AI";
        const verdict =
          reviewByAi && task.review?.decision === "Approved"
            ? "approved"
            : reviewByAi && task.review?.decision === "Rejected"
              ? "rejected"
              : Number.isFinite(score) && score >= thresholds.autoApproveThreshold
                ? "approved"
                : Number.isFinite(score) && score < thresholds.autoRejectThreshold
                  ? "rejected"
        ...
        if (verdict === "rejected") acc.rejected += 1;
        if (verdict === "pending") acc.pending += 1;
        if (verdict === "needsReview") acc.needsReview += 1;
        return acc;
      },
      { approved: 0, rejected: 0, pending: 0, needsReview: 0 },
    );
  }, [tasks]);
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="task-panel p-6 sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-blue-500">DTMS workflow</p>
            <h2 className="mt-3 text-2xl font-semibold text-slate-950">Explore the full AI hiring flow</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              See how project evaluation, screening, interviews, emails, and deadline automation connect in one system.
            </p>
          </div>
          <button
            type="button"
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <MetricCard
          label="AI Approved"
          value={aiStats.approved}
          detail="Submissions that met the automatic approval threshold."
          accent="green"
        />
        <MetricCard
          label="AI Pending"
          value={aiStats.pending}
          detail="AI-scored submissions waiting for admin confirmation."
          accent="cyan"
        />
        <MetricCard
          label="AI Rejected"
          value={aiStats.rejected + aiStats.needsReview}
          detail="Submissions that were auto-rejected or flagged for closer review."
          accent="rose"
        />
      </section>
