...
export default function UserDashboardPage() {
  ...
  const latestSubmittedTask = useMemo(
    () =>
      [...tasks]
        .filter((task) => task.submission || normalizeTaskStatus(task.status) === "COMPLETED" || String(task.review?.decision || "").toLowerCase() === "approved")
        .sort((left, right) => new Date(right.updatedAt || right.createdAt || right.deadline || Date.now()) - new Date(left.updatedAt || left.createdAt || left.deadline || Date.now()))[0] || null,
    [tasks],
  );
  const screeningUnlocked = Boolean(latestSubmittedTask);
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="task-panel p-6 sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-blue-500">Candidate workflow</p>
            <h2 className="mt-3 text-2xl font-semibold text-slate-950">
              {screeningUnlocked ? "Screening interview round unlocked" : "Submit your project to unlock screening"}
            </h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              {screeningUnlocked
                ? "Your submitted project can now move to the screening interview round."
                : "Once you submit a project, DTMS will show the screening interview round here."}
            </p>
          </div>
          <button
            type="button"
            onClick={() => navigate("/dtms-workflow")}
            className="rounded-2xl bg-[linear-gradient(90deg,#2563eb_0%,#60a5fa_100%)] px-5 py-3 text-sm font-semibold text-white shadow-[0_18px_40px_rgba(37,99,235,0.22)] transition hover:-translate-y-0.5"
          >
            View workflow
          </button>
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <MetricCard
          ...
          value={screeningUnlocked ? "Submitted" : "Not submitted"}
          detail={screeningUnlocked ? `Latest submission: ${latestSubmittedTask?.title || "Task"}` : "Submit a project to unlock screening."}
          accent={screeningUnlocked ? "green" : "rose"}
        />
        <MetricCard
          label="Screening interview"
          value={screeningUnlocked ? "Visible" : "Hidden"}
          detail={screeningUnlocked ? "The screening round is ready from your dashboard." : "It appears after a project is submitted."}
          accent={screeningUnlocked ? "cyan" : "slate"}
        />
      </section>
