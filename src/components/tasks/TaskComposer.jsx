...
export default function TaskComposer({
  ...
  const selectedUser = useMemo(
    () => users.find((user) => user.id === form.assignedUserId),
    [form.assignedUserId, users],
  );
        ...
        <div className="mt-8 grid gap-5">
          ...
          <div className="grid gap-4 rounded-3xl border border-slate-200/80 bg-[linear-gradient(145deg,rgba(79,70,229,0.05),rgba(6,182,212,0.04),rgba(255,255,255,0.92))] p-5 md:grid-cols-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-700">{selectedUser?.name || "No teammate selected yet"}</p>
            </div>
            <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <p className="mt-2 text-sm font-medium text-slate-950">
                {selectedUser?.name || "Choose a teammate"}
              </p>
              <p className="mt-1 text-sm text-slate-600">{selectedUser?.email || "No assignee selected"}</p>
            </div>
